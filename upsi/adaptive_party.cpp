...
void AdaptiveParty<BaseType>::refresh_oprfs() {
    ...
    for (int i = 0; i <= my_adaptive.node_cnt; ++i) {
        ...
        for (const Element& cur_elem: tmp) {
            ASE_ind.push_back(i);
            points.push_back(my_adaptive_encrypted.nodes[i]->findPos(cur_elem, false));
        }
    ...
    for (int i = 0; i < elems.size(); ++i)
        values.push_back(oprf.receiver_plain(elems[i], ASE_ind[i], my_vole.nodes[ASE_ind[i]]->ase[points[i]], my_adaptive.seeds[ASE_ind[i]]));
